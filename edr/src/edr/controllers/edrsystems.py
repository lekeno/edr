...
from .edrcache import EDRCacheManager
...
class EDRSystems:
    ...
    def __init__(self, server, edsm_server, factions):
        ...
        self.cache_manager = EDRCacheManager()
        
        # --- EDR Caches ---
        self.systems_cache = self.cache_manager.systems_cache
        self.materials_cache = self.cache_manager.materials_cache
        self.notams_cache = self.cache_manager.notams_cache
        self.sitreps_cache = self.cache_manager.sitreps_cache
        self.crimes_cache = self.cache_manager.crimes_cache

        # --- EDR Fleet Carrier (FC) Caches ---
        self.fc_reports_cache = self.cache_manager.fc_reports_cache
        self.fc_presence_cache = self.cache_manager.fc_presence_cache
        self.fc_materials_cache = self.cache_manager.fc_materials_cache
        self.fcs_cache = self.cache_manager.fcs_cache
        self.traffic_cache = self.cache_manager.traffic_cache

        # --- EDSM Caches ---
        self.edsm_systems_cache = self.cache_manager.edsm_systems_cache
        self.edsm_bodies_cache = self.cache_manager.edsm_bodies_cache
        self.edsm_stations_cache = self.cache_manager.edsm_stations_cache
        self.edsm_systems_within_radius_cache = self.cache_manager.edsm_systems_within_radius_cache
        self.edsm_traffic_cache = self.cache_manager.edsm_traffic_cache
        self.edsm_markets_cache = self.cache_manager.edsm_markets_cache
        self.edsm_shipyards_cache = self.cache_manager.edsm_shipyards_cache
        self.edsm_outfitting_cache = self.cache_manager.edsm_outfitting_cache
        self.edsm_system_values_cache = self.cache_manager.edsm_system_values_cache
        self.edsm_deaths_cache = self.cache_manager.edsm_deaths_cache
