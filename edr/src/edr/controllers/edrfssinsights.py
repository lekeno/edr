...
class EDRFSSInsights:
    """
    Tracks and summarizes Full Spectrum System Scanner (FSS) discoveries.
    """
    ...
    def __init__(self):
        ...
        self.resource_extraction_sites = {
            "available": False,
            "variants": {
                "$MULTIPLAYER_SCENARIO14_TITLE;": {"count": 0, "short_name": _c("Standard Res|Std")},
                "$MULTIPLAYER_SCENARIO77_TITLE;": {"count": 0, "short_name": _c("Res Low|Low")},
                "$MULTIPLAYER_SCENARIO78_TITLE;": {"count": 0, "short_name": _c("Res High|High")},
                "$MULTIPLAYER_SCENARIO79_TITLE;": {"count": 0, "short_name": _c("Res Hazardous|Haz")}
            },
            "short_name": _("RES")
        }
        self.combat_zones = {
            "available": False,
            "variants": {
                "$Warzone_PointRace_Low;": {"count": 0, "short_name": _c("CZ Low intensity|Low")},
                "$Warzone_PointRace_Medium;": {"count": 0, "short_name": _c("CZ Medium intensity|Med")},
                "$Warzone_PointRace_High;": {"count": 0, "short_name": _c("CZ High intensity|High")}
            },
            "short_name": _("CZ")
        }
        self.uss = {
            "available": False,
            "variants": {
                "$USS_Type_Salvage;": {"count": 0, "expiring": [], "short_name": _c("Degraded Emissions|Degraded")},
                "$USS_Type_ValuableSalvage;": {"count": 0, "expiring": [], "short_name": _c("Encoded Emissions|Encoded")},
                "$USS_Type_VeryValuableSalvage;": {"count": 0, "expiring": [], "short_name": _c("High Grade Emissions|High Grade")},
                "misc": {"count": 0, "expiring": [], "short_name": _c("Misc.")}
            },
            "short_name": _("USS")
        }
        self.pirates = {
            "available": False,
            "variants": {
                "$FIXED_EVENT_HIGHTHREATSCENARIO_T7;": {"count": 0, "short_name": _c("Pirates Threat 7|Th7")},
                "$FIXED_EVENT_HIGHTHREATSCENARIO_T6;": {"count": 0, "short_name": _c("Pirates Threat 6|Th6")},
                "$FIXED_EVENT_HIGHTHREATSCENARIO_T5;": {"count": 0, "short_name": _c("Pirates Threat 5|Th5")}
            },
            "short_name": _("Pirates")
        }
    ...
    def reset(self, override_timestamp=None):
        """
        Reset the state of the FSS insights, potentially overriding the timestamp.

        Args:
            override_timestamp (str, optional): The timestamp string to use for reset.
        """
    ...
    def related_to(self, current_star_system):
        """
        Check if the current insights are related to the given star system.

        Args:
            current_star_system (str): The name of the star system.

        Returns:
            bool: True if related or system name is not set, False otherwise.
        """
        return (self.star_system["name"] is None) or current_star_system == self.star_system["name"]

    def update(self, current_star_system):
        """
        Update the current star system name, resetting if changed.

        Args:
            current_star_system (str): The new star system name.
        """
    ...
    def update_system(self, system_address, system_name):
        """
        Update system details using address and name.

        Args:
            system_address (int): The system address.
    ...
    def same_system(self, system_address):
        """
        Check if the given system address matches the current one.

        Args:
            system_address (int): The system address to check.
    ...
    def process(self, fss_event):
        """
        Process an FSS signal discovery event.

        Args:
            fss_event (dict): The journal event data.

        Returns:
            bool: True if the event was processed and noteworthy, False otherwise.
        """
    ...
    def summarize(self):
        """
        Generate a summary of the discovered signals.
    ...
    def fleet_carriers_report(self, force_reporting=False):
        """
        Generate a report of fleet carriers in the system.

        Args:
            force_reporting (bool): Whether to force reporting even if no system or not processed.

        Returns:
            dict: A dictionary containing the fleet carrier report, or None.
        """
    ...
    def fuzzy_match_fleet_carriers(self, callsign_or_name):
        """
        Fuzzy match fleet carriers by callsign or name.

        Args:
            callsign_or_name (str): The callsign or name to match.

        Returns:
            dict: A dictionary of matched fleet carriers.
        """
    ...
    def is_signal(self, name):
        """
        Check if the given name is a known signal.

        Args:
            name (str): The name to check.

        Returns:
            bool: True if it is a signal, False otherwise.
        """
        return (name in self.signals_seen) or self.is_scenario_signal(name)

    def no_signals(self):
        """
        Check if no signals have been seen.

        Returns:
            bool: True if no signals seen, False otherwise.
        """
        return len(self.signals_seen) == 0

    def is_scenario_signal(self, name):
        """
        Check if the name corresponds to a scenario signal (starting with $ and ending with ;).

        Args:
            name (str): The name to check.

        Returns:
            bool: True if it is a scenario signal, False otherwise.
        """
        return bool(re.search(r'^\$[ -~]+;$', name))

    def is_nav_beacon(self, name):
        """
        Check if the name corresponds to a Nav Beacon.

        Args:
            name (str): The name to check.

        Returns:
            bool: True if it is a Nav Beacon, False otherwise.
        """
        return name in ["$MULTIPLAYER_SCENARIO42_TITLE;", "$MULTIPLAYER_SCENARIO80_TITLE;"]

    def is_uss(self, name):
        """
        Check if the name corresponds to a USS (Unidentified Signal Source).

        Args:
            name (str): The name to check.

        Returns:
            bool: True if it is a USS, False otherwise.
        """
        return name in ["$USS_Type_Salvage;", "$USS_Type_ValuableSalvage;", "$USS_Type_VeryValuableSalvage;"]

    def is_station(self, name):
        """
        Check if the name corresponds to a known station.

        Args:
            name (str): The name to check.

        Returns:
            bool: True if it is a station, False otherwise.
        """
        return name in self.stations

    def is_main_star(self, name):
        """
        Check if the name corresponds to the main star of the system.

        Args:
            name (str): The name to check.

        Returns:
            bool: True if it is the main star, False otherwise.
        """
        return name == self.star_system.get("name", None)

    def newly_found_fleet_carriers(self):
        """
        Get newly found fleet carriers since the last check and clear the recent list.

        Returns:
            dict: A dictionary of newly found fleet carriers.
        """
