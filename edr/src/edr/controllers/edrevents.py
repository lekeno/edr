...
class EDREventHandler(object):
    def __init__(self, edr_client):
        self.edr_client = edr_client
        self.last_known_ship_name = ''
        self.in_legacy_mode = False
        self.journal_entry_first_run = dict()

    def prerequisites(self, edr_client, is_beta, is_legacy):
    ...
    def handle_wing_events(self, ed_player, entry):
        if entry["event"] in ["WingAdd"]:
            wingmate = self.plain_cmdr_name(entry["Name"])
            ...
            requester = self.plain_cmdr_name(entry["Name"])
    ...
    def handle_multicrew_events(self, ed_player, entry):
        if entry["event"] in ["CrewMemberJoins", "CrewMemberRoleChange", "CrewLaunchFighter"]:
            crew = self.plain_cmdr_name(entry["Crew"])
        ...
        if entry["event"] in ["CrewMemberQuits", "KickCrewMember"]:
            ...
            self.edr_submit_multicrew_session(ed_player, report)

        if entry["event"] in ["JoinACrew"]:
            captain = self.plain_cmdr_name(entry["Captain"])
    ...
    def dashboard_entry(self, cmdr, is_beta, entry):
        ...
        if not self.prerequisites(self.edr_client, is_beta, self.in_legacy_mode):
            return
    ...
    def journal_entry(self, cmdr, is_beta, system, station, entry, state):
        ...
        if entry["event"] in ["Shutdown", "ShutDown", "Music", "Resurrect", "Fileheader", "LoadGame", "Loadout", "SuitLoadout", "SwitchSuitLoadout", "LaunchSRV", "DockSRV", "Disembark", "Embark", "DropShipDeploy"]:
            from_genesis = False
            if "first_run" not in self.journal_entry_first_run:
                self.journal_entry_first_run = False
                from_genesis = (entry["event"] == "LoadGame") and (cmdr and system is None and station is None)
            self.handle_lifecycle_events(ed_player, entry, state, from_genesis)
        ...
        if entry["event"] == "Friends":
            self.handle_friends_events(ed_player, entry)

        if entry["event"] == "EngineerProgress":
            self.handle_engineer_progress(ed_player, entry)
        ...
        if entry["event"] in ["Materials", "MaterialCollected", "MaterialDiscarded", "EngineerContribution", "EngineerCraft", "MaterialTrade", "MissionCompleted", "ScientificResearch", "TechnologyBroker", "Synthesis", "Backpack", "BackpackChange", "BuyMicroResources", "SellMicroResources", "TransferMicroResources", "TradeMicroResources", "ShipLockerMaterials", "ShipLocker"]:
            self.handle_material_events(ed_player, entry, state)

        if entry["event"] == "StoredShips":
            ed_player.update_fleet(entry)

        if "Crew" in entry["event"]:
            self.handle_multicrew_events(ed_player, entry)
        
        if entry["event"] in ["WingAdd", "WingJoin", "WingLeave"]:
            self.handle_wing_events(ed_player, entry)

        if entry["event"] in ["MiningRefined", "ProspectedAsteroid"]:
            self.handle_mining_events(ed_player, entry)
    
        if entry["event"] == "Bounty":
            self.handle_bounty_hunting_events(ed_player, entry)
        ...
        if entry["event"] in ["CarrierJump", "CarrierBuy", "CarrierStats", "CarrierJumpRequest", "CarrierJumpCancelled", "CarrierDecommission", "CarrierCancelDecommission", "CarrierDockingPermission", "CarrierTradeOrder", "FCMaterials"]:
            self.handle_carrier_events(ed_player, entry)
        ...
        if entry["event"] in ["Location", "Undocked", "Docked", "DockingCancelled", "DockingDenied",
            ...
            outcome = self.handle_change_events(ed_player, entry)
            self.handle_fc_position_related_events(ed_player, entry)
            if outcome["updated"]:
        ...
        if entry["event"] in ["SupercruiseExit", "FSDJump", "SupercruiseEntry", "StartJump",
            ...
            outcome = self.handle_movement_events(ed_player, entry)
            if outcome["updated"]:
                status_outcome["updated"] = True
                status_outcome["reason"] = outcome["reason"]
        ...
        if entry["event"] in ["HullDamage", "UnderAttack", "SRVDestroyed", "FighterDestroyed", "HeatDamage", "ShieldState", "CockpitBreached", "SelfDestruct"]:
            self.handle_damage_events(ed_player, entry)

        if entry["event"] in ["FuelScoop", "RefuelAll", "RefuelPartial", "Repair", "RepairAll", "AfmuRepairs", "RebootRepair", "RepairDrone"]:
            self.handle_fixing_events(ed_player, entry)

        if entry["event"] in ["ModuleStore", "ModuleSell", "ModuleBuy", "ModuleRetrieve", "MassModuleStore"]:
            self.handle_outfitting_events(ed_player, entry)

        if entry["event"] in ["ReceiveText", "SendText"]:
            self.report_comms(ed_player, entry)

        if entry["event"] in ["SendText"]:
            self.handle_commands(ed_player, entry)

        if entry["event"] == "MissionAccepted":
            self.handle_mission_events(ed_player, entry)

        if status_outcome["updated"]:
            self.edr_update_cmdr_status(ed_player, status_outcome["reason"], entry["timestamp"])
            if ed_player.in_a_crew():
                for member in ed_player.crew.members:
                    ...
                    self.edr_submit_contact(crew_player, entry["timestamp"], source, ed_player)
    
        if ed_player.maybe_in_a_pvp_fight():
            self.report_fight(ed_player)
    ...
    def report_crime(self, cmdr, entry):
        ...
        if entry["event"] in ["Interdicted", "EscapeInterdiction"]:
            if entry["IsPlayer"]:
                ...
                self.edr_submit_crime([interdictor], entry["event"], cmdr, entry["timestamp"])
            else:
            ...
            if "Killers" in entry:
                ...
                self.edr_submit_crime(criminal_cmdrs, "Murder", cmdr, entry["timestamp"])
                ...
                self.edr_submit_crime([criminal_cmdr], "Murder", cmdr, entry["timestamp"])
            player_one.killed()
            ...
            if entry["IsPlayer"]:
                ...
                self.edr_submit_crime_self(cmdr, offence, interdicted, entry["timestamp"])
            else:
                offence = "Interdiction" if entry["Success"] else "Failed interdiction"
    ...
    def report_comms(self, player, entry):
        ...
        if entry["event"] == "ReceiveText" and "Channel" in entry:
            if entry["Channel"] in ["local"]:
                ...
                self.edr_submit_contact(contact, entry["timestamp"], "Received text (local)", player)
                ...
                if m:
                    ...
                    self.edr_submit_contact(contact, entry["timestamp"], "Emote sent (non wing/friend player)", player)
    ...
    def handle_scan_events(self, player, entry):
        ...
        if target.is_human():
            self.edr_submit_contact(target, entry["timestamp"], "Ship targeted", player)
        ...
        if entry["ScanStage"] == 3:
            ...
            if target.is_human():
                self.edr_submit_scan(scan, entry["timestamp"], "Ship targeted [{}]".format(entry["LegalStatus"]), player)
