...
class EDRBodiesOfInterest:
    def __init__(self, dlc=None):
        ...
        self.boi = json.loads(open(os.path.join(os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', '..')), 'data', filename)).read())
        self.custom_pois = {}
    ...
    def add_custom_poi(self, star_system, body_name, poi):
        if not star_system or not body_name:
            EDR_LOG.warning(f"Can't add poi: no system {star_system} or no body {body_name}")
            return False
        c_star_system = star_system.lower()
        c_body_name = simplified_body_name(star_system, body_name)
    ...
    def reset_custom_poi(self, star_system, body_name):
        self.index_custom_pois = {}
        if not star_system or not body_name:
            EDR_LOG.warning(f"Can't clear custom POIs, no system {star_system} or no body {body_name}")
            return None
        c_star_system = star_system.lower()
        c_body_name = simplified_body_name(star_system, body_name)
        if c_star_system not in self.custom_pois:
            EDR_LOG.warning(f"System has no custom POIs: {c_star_system}, {self.custom_pois}")
            return
        
        if c_body_name not in self.custom_pois[c_star_system]:
            EDR_LOG.warning(f"Body has no custom POIs: {c_body_name}, {self.custom_pois}")
            return
    ...
    def custom_points_of_interest(self, star_system, body_name):
        if not star_system or not body_name:
            EDR_LOG.warning(f"No star system or body name: {star_system}, {body_name}")
            return None
        c_star_system = star_system.lower()
        c_body_name = simplified_body_name(star_system, body_name)
    ...
    def closest_custom_point_of_interest(self, star_system, body_name, attitude, planet_radius):
        ...
        if not pois:
            return None
        if not planet_radius:
            EDR_LOG.warning("No planet radius, sending back first poi")
            return pois[0]
        closest = {"distance": None, "poi": None}
        i = 0
        for poi in pois:
