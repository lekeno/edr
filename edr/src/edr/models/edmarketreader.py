...
class EDMarketReader:
    """
    Reads the Market.json file from the journal directory.
    """
    ...
    def process(self):
        """Read and parse the Market.json file.

        Returns:
            dict: The parsed JSON data as a dictionary, or None if failed.
        """
        try:
            with open(join(self.journal_location, 'Market.json'), 'rb') as handle:
                data = handle.read().strip()
                if data:  # Can be empty if polling while the file is being re-written
                    entry = json.loads(data)
                    return entry
        except Exception:
            EDR_LOG.exception("Couldn't process market")
            return None
