...
class EDRSystemSettlementCheck(object):
    ...
    def check_settlement(self, settlement, system_name=None):
        ...
        EDR_LOG.debug("Checking: {}".format(settlement['name']))

        EDR_LOG.debug(settlement)
        if "settlement" not in settlement.get("type", "").lower():
            EDR_LOG.debug("no settlement")
            return False
        
        self.settlements_counter = self.settlements_counter + 1
        if settlement.get('distanceToArrival', None) is None:
            EDR_LOG.debug("no distance")
            return False

        EDR_LOG.debug("Dist: {} vs {}".format(settlement['distanceToArrival'], self.max_sc_distance))
...
class EDRSystemOdySettlementCheck(EDRSystemSettlementCheck):
    ...
    def check_settlement(self, settlement, system_name=None):
        ...
        if settlement.get("type", "").lower() != "odyssey settlement":
            ...
            EDR_LOG.debug("not an odyssey settlement")
            return False
...
class EDROdySettlementCheck(EDRSystemOdySettlementCheck):
    ...
    def check_settlement(self, settlement, system_name=None):
        ...
        if self.economies:
            if not eco or eco.lower() not in self.economies:
                EDR_LOG.debug("Eco not matching any required state for the controlling faction: {} vs. {}".format(eco, self.economies))
                return False
            
        if self.exclude_economies:
            if not eco or eco.lower() in self.exclude_economies:
                EDR_LOG.debug("Eco matching an exluded state for the controlling faction: {} vs. {}".format(eco, self.exclude_economies))
                return False
    
        gvt = settlement.get('government', None)
        if self.governments:
            if not gvt or gvt.lower() not in self.governments:
                EDR_LOG.debug("Gvt not matching any required state for the controlling faction: {} vs. {}".format(gvt, self.governments))
                return False
            
        if self.exclude_governments:
            if not gvt or gvt.lower() in self.exclude_governments:
                EDR_LOG.debug("Gvt matching an excluded state for the controlling faction: {} vs. {}".format(gvt, self.exclude_governments))
                return False
                
        alg = settlement.get('allegiance', None)
        if self.allegiances:
            if not alg or alg.lower() not in self.allegiances:
                EDR_LOG.debug("Alg not matching any required state for the controlling faction: {} vs. {}".format(alg, self.allegiances))
                return False
            
        if self.exclude_allegiances:
            if not alg or alg.lower() in self.exclude_allegiances:
                EDR_LOG.debug("Alg matching an excluded state for the controlling faction: {} vs. {}".format(alg, self.exclude_allegiances))
                return False
        ...
        EDR_LOG.debug("Checking faction: {}".format(faction))
        state = faction.state if faction else None
        
        if self.bgs_states:
            if not faction:
                EDR_LOG.debug("No faction info => can't match against BGS conditions")
                return False
            
            if state not in self.bgs_states:
                EDR_LOG.debug("BGS not matching any required state for the controlling faction: {} vs. {}".format(state, self.bgs_states))
                return False
            
        if self.exclude_bgs_states:
            if not faction:
                EDR_LOG.debug("No faction info => can't match against BGS exclusions")
                return False
            
            if state in self.exclude_bgs_states:
                EDR_LOG.debug("BGS matching an excluded state for the controlling faction: {} vs. {}".format(state, self.exclude_bgs_states))
                return False
    ...
    def is_ambiguous(self, settlement, system_name=None):
        ...
        if not reference:
            EDR_LOG.debug("Faction {} is ambiguous because its BGS state is unknown (no update timestamp)".format(factionName))
            return True
        ...
        if too_old:
            EDR_LOG.debug("Faction {} is ambiguous because its last known BGS state is too old: {}".format(factionName, EDTime.t_minus_py(reference)))
            return True
...
class EDRSettlementCheckerFactory(object):
    ...
    def get_checker(words_salad, override_sc_distance, edrsystems):
        ...
        for word in words:
            ...
            EDR_LOG.debug("unknown qualifier: {}".format(word))
